...
// import { supabase } from "../../../../backend/api/src/shared/database/supabaseClient";
...
export default function AgentModule() {
  ...
  // 🛡️ SECURITY: Check if Agent is logged in
  useEffect(() => {
  ...
  const fetchData = async () => {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}/meters/assigned?worker_id=${workerId}`,
      );
        ...
        const data =
          activeTab === "pending"
            ? result.data.filter(
                (m) =>
                  !m.last_reading_at ||
                  new Date(m.last_reading_at).toDateString() !==
  ...
  const handleSubmit = async (e) => {
    ...
    if (!currentReading) return alert("Reading is necessary!");
    if (!image) return alert("Photo is necessary!");

    // Validation: Reading must be higher than last reading
    if (parseFloat(currentReading) <= (selectedMeter.last_reading || 0)) {
      return alert(
    ...
    try {
      ...
      const fileName = `${Date.now()}_${workerId}.${fileExt}`;
      const filePath = `meter-readings/${fileName}`;

      const { error: uploadError } = await supabase.storage
        .from("biogrix-assets") // ⚠️ Make sure this bucket exists in Supabase
      ...
      const response = await fetch(`${API_BASE_URL}/gas-usage/submit`, {
        ...
        body: JSON.stringify({
          ...
          image_url: publicUrl, // 👈 Saved Cloud Link
          entered_by: workerId,
        }),
      });
          ...
          <div className="flex justify-between items-center">
            <div>
              <h2 className="text-2xl font-black text-neutral-900 tracking-tighter">
                <span className="uppercase">Biogrix</span>
                <span className="text-primary">.</span>
                <span className="ml-4 text-primary">Agent</span>
              </h2>
              <p className="text-[9px] font-bold text-neutral-400 uppercase tracking-widest mt-0.5 italic">
                Welcome, {workerName}
              </p>
            </div>
            <button
              onClick={handleLogout}
              className="w-10 h-10 bg-white border border-neutral-100 rounded-2xl flex items-center justify-center text-red-500 shadow-sm active:scale-90 transition-all"
            >
              <LogOut size={18} />
            </button>
          </div>
        ...
        {!selectedMeter ? (
          ...
          <div className="space-y-4 py-4">
            {loading ? (
              ...
              <div className="py-20 text-center space-y-2">
                <CheckCircle2
                  size={40}
                  className="mx-auto text-emerald-400 opacity-20"
                />
              ...
              filtered.map((meter) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    <div
                      className={`w-12 h-12 rounded-2xl flex items-center justify-center ${activeTab === "pending" ? "bg-red-50 text-red-500" : "bg-primary/5 text-primary"}`}
                    >
                      <Gauge size={24} />
                    </div>
                    <div>
                      <h4 className="font-black text-neutral-800 text-sm">
          ...
          <div className="animate-in slide-in-from-bottom-10 duration-300 py-6">
            <button
              ...
              className="mb-6 flex items-center gap-2 text-xs font-black uppercase text-neutral-400 hover:text-primary transition-colors"
            >
              ...
              <div className="flex justify-between items-start relative z-10">
                <div>
                  ...
                  <h3 className="text-2xl font-black leading-tight tracking-tight">
                    {selectedMeter.customers?.name}
                  </h3>
                  <div className="flex flex-col gap-1 mt-2">
                    <p className="text-[10px] text-neutral-400 font-black flex items-center gap-1 uppercase">
                      <MapPin size={10} /> {selectedMeter.serial_number}
                    </p>
                    <p className="text-[10px] text-emerald-500 font-black flex items-center gap-1 uppercase">
                      <Activity size={10} /> Grid Link Active
                    </p>
                  </div>
                </div>
                <div className="bg-white/5 p-4 rounded-3xl border border-white/5 text-right">
                  <p className="text-[8px] font-black uppercase text-neutral-500 mb-1">
                    Previous
                  </p>
                  <p className="text-xl font-black text-primary tracking-tighter">
                    {selectedMeter.last_reading || "0.00"}
                  </p>
                </div>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-8 relative z-10">
                ...
                <div className="space-y-3">
                  ...
                  <input
                    ...
                    capture="environment"
                    className="hidden"
                    id="meter-photo-agent"
                    onChange={(e) => setImage(e.target.files[0])}
                  />
                  <label
                    htmlFor="meter-photo-agent"
                    className="block cursor-pointer"
                  >
                    <div
                      className={`border-2 border-dashed rounded-[2rem] p-10 flex flex-col items-center justify-center transition-all ${image ? "border-primary bg-primary/10" : "border-neutral-800 bg-white/5 hover:border-primary"}`}
                    >
                      {image ? (
                        <div className="text-center">
                          ...
                          <span className="text-[10px] font-black text-white uppercase tracking-widest">
                            Captured: {image.name.slice(0, 10)}...
                          </span>
                        </div>
                      ) : (
                        <>
                    ...
                    <div className="mt-4 p-5 bg-primary/10 rounded-2xl border border-primary/20 text-center animate-in zoom-in">
                      ...
                      <p className="text-3xl font-black text-white">
                        {(
                          ...
                          (selectedMeter.last_reading || 0)
                        ).toFixed(2)}
                        <span className="text-xs text-neutral-500 ml-2 font-bold uppercase">
                          Units
                        </span>
                      </p>
                    </div>
