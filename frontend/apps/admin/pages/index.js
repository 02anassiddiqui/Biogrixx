...
export default function AdminDashboard() {
  ...
  const fetchLeads = async () => {
    setLoading(true);
    try {
      const response = await fetch("http://localhost:4000/v1/customers", {
        headers: {
          // 🚀 Hardcoded password ki jagah ab hum browser se chabi le rahe hain
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`http://localhost:4000/v1/customers/${id}`, {
        method: "DELETE",
        headers: {
          // 🚀 Yahan bhi wahi browser storage waali chabi jayegi
          "x-admin-secret": localStorage.getItem("biogrix_admin_key"),
        },
      });
      const result = await response.json();
  ...
  // Logout Function
  const handleLogout = () => {
    localStorage.removeItem("biogrix_admin_key"); // Chabi delete kar di
    router.push("/login"); // Login page par bhej diya
  };

  const router = useRouter(); // router import karna mat bhoolna

  useEffect(() => {
    const key = localStorage.getItem("biogrix_admin_key");
    if (!key) {
      router.push("/login"); // Agar login nahi hai toh login page par bhejo
    } else {
      fetchLeads(); // Agar login hai toh data mangwao
    }
  }, []);

  // Initials nikalne ke liye helper function
  ...
  return (
    <div className="min-h-screen bg-white font-sans selection:bg-emerald-100">
      ...
      <div className="bg-white pt-16 pb-10">
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex flex-col md:flex-row md:items-end justify-between">
            <div>
              <div className="px-4 py-1.5 rounded-full bg-emerald-50 text-primary text-[10px] font-black uppercase tracking-[0.2em] mb-6 border border-emerald-100 inline-block">
            ...
            <div className="flex gap-4">
              <button
                onClick={fetchLeads}
                className="group flex items-center gap-3 bg-neutral-900 text-white px-8 py-5 rounded-2xl font-black uppercase tracking-widest text-[10px] hover:bg-primary transition-all shadow-2xl active:scale-95"
              >
                <RefreshCw
                  size={16}
                  className={
              ...
              <button
                ...
                className="flex items-center bg-primary px-8 py-5 rounded-2xl gap-2 text-white hover:bg-red-600 font-black uppercase text-[10px] tracking-widest transition-all"
              >
      ...
      {/* --- STATS SECTION --- */}
      <div className="bg-gradient-to-r from-primary to-emerald-600 py-16 relative overflow-hidden rounded-t-[4rem]">
          ...
          <div className="p-8 border-b border-neutral-50 flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <div className="relative w-full md:w-72">
              ...
              <input
                ...
                className="w-full pl-11 pr-6 py-3 bg-neutral-50 border-none rounded-xl text-[10px] font-black tracking-widest outline-none focus:ring-2 focus:ring-primary/10 transition-all uppercase"
              />
            </div>
          </div>
                  ...
                  >
                    ...
                    <td className="px-6 py-5">
                      <div className="flex items-end gap-1">
                        ...
                        <span className="text-[8px] font-black text-primary uppercase mb-0.5 tracking-tighter">
                          Heads
                        </span>
                      </div>
                    </td>
