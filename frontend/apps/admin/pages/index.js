  ...
  const fetchLeads = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        const todayCount = result.data.filter(l => l.created_at.startsWith(today)).length;
          ...
          <div className="overflow-x-auto">
            {loading ? (
              <div className="py-20 flex flex-col items-center justify-center gap-4">
                ...
                <p className="text-[10px] font-black uppercase tracking-widest text-neutral-400">Loading Infrastructure...</p>
              </div>
            ) : (
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-neutral-50">
                  {filteredLeads.map((lead) => (
                    <tr key={lead.id} className="hover:bg-emerald-50/10 transition-all group">
                      <td className="px-6 py-5">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="font-black text-neutral-900 uppercase text-xs tracking-tight">{lead.name}</div>
                            <div className="text-[9px] text-neutral-400 font-bold flex items-center gap-1 mt-0.5">
                              <Phone size={8} className="text-emerald-500" /> {lead.phone}
                            </div>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-5">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-[11px] font-black text-neutral-700 uppercase">{lead.village}</span>
                        </div>
                      </td>
                      <td className="px-6 py-5">
                        <div className="flex items-end gap-1">
                          <span className="text-xl font-black text-neutral-900 leading-none">{lead.livestock_count}</span>
                          <span className="text-[8px] font-black text-emerald-500 uppercase mb-0.5 tracking-tighter">Heads</span>
                        </div>
                      </td>
                      <td className="px-6 py-5">
                        <p className="text-[10px] text-neutral-500 italic truncate max-w-[150px]">{lead.message || "---"}</p>
                      </td>
                      <td className="px-6 py-5 text-right">
                        <div className="flex items-center justify-end gap-4">
                          ...
                          <button onClick={() => handleDelete(lead.id)} className="p-2 text-neutral-200 hover:text-red-500 transition-all">
                            <Trash2 size={16} />
                          </button>
                        </div>
                      </td>
...
function StatCard({ label, value, icon }) {
  return (
    <div className="bg-white/10 backdrop-blur-xl border border-white/20 p-8 rounded-[2.5rem] flex flex-col justify-between h-44 group hover:bg-white/15 transition-all">
      ...
      <div>
        <div className="text-4xl font-black text-white tracking-tighter mb-1.5">{value}</div>
        <div className="text-[10px] uppercase font-black tracking-[0.2em] text-emerald-100/60 leading-none">{label}</div>
      </div>
    </div>
  );
}